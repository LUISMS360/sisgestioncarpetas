<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{$title ?? config('app.name')}}</title>
    @vite(['resources/css/app.css','resources/js/app.js'])
    <link rel="shortcut icon" href="{{ asset('assets/compiled/svg/favicon.svg') }}" type="image/x-icon">
    <link rel="shortcut icon" href="data:image/png;base64," type="image/png">
    <link rel="stylesheet" href="{{ asset('/assets/compiled/css/app.css') }}">
    <link rel="stylesheet" href="{{ asset('assets/compiled/css/app-dark.css') }}">
    <link rel="stylesheet" href="{{ asset('assets/compiled/css/iconly.css') }}">
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    @livewireStyles
</head>

<body class="">
    <script src="{{ asset('assets/static/js/initTheme.js') }}"></script>
    <div id="app">
        <div id="sidebar">
            <div class="sidebar-wrapper active">
                <div class="sidebar-header position-relative">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="logo" style="height: 100px">
                            <a href="{{ route('admin.dashboard') }}">
                                <img src="{{ asset('assets/img/logodsi.png') }}" alt="Logo" class="h-100 d-inline-block">
                            </a>
                        </div>
                        <div>
                            <span class="small fw-bold fs-5 text">SISGESTION</span>
                        </div>
                    </div>
                </div>
                <div class="sidebar-menu">
                    <ul class="menu">
                        <li class="sidebar-item">
                            <livewire:frontend.components.notificaciones.admin />
                        </li>
                        <li class="sidebar-title">Menu</li>

                        <li
                            class="sidebar-item">
                            <a href="{{ route('admin.dashboard') }}" class='sidebar-link'>
                                <i class="bi bi-grid-fill"></i>
                                <span>Tramites</span>
                            </a>


                        </li>

                        <li
                            class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-stack"></i>
                                <span>Futs</span>
                            </a>

                            <ul class="submenu ">

                                <li class="submenu-item  ">
                                    <a href="{{ route('admin.futs.pendientes') }}" wire:navigate class="submenu-link">Pendientes</a>

                                </li>
                                <li class="submenu-item  ">
                                    <a href="component-accordion.html" class="submenu-link">Revisados</a>

                                </li>

                            </ul>

                        </li>

                        <li
                            class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-collection-fill"></i>
                                <span>Carpetas</span>
                            </a>

                            <ul class="submenu">

                                <li class="submenu-item  ">
                                    <a href="{{ route('admin.crear-carpeta') }}" wire:navigate class="submenu-link">Crear</a>
                                </li>
                                <li class="submenu-item  ">
                                    <a href="{{ route('admin.carpetas.pendientes') }}" wire:navigate class="submenu-link">Pendientes</a>
                                </li>
                                <li class="submenu-item">
                                    <a href="{{ route('admin.carpetas-gestionadas') }}" wire:navigate class="submenu-link">Gestionadas</a>
                                </li>
                            </ul>
                        </li>

                        <li
                            class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-file-earmark-text-fill"></i>
                                <span>Actas</span>
                            </a>
                            <ul class="submenu ">
                                <li class="submenu-item">
                                    <a href="" class="submenu-link">Gestionadas</a>
                                </li>
                            </ul>
                        </li>

                        <li
                            class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-file-earmark-check-fill"></i>
                                <span>Constancias</span>
                            </a>
                            <ul class="submenu ">
                                <li class="submenu-item">
                                    <a href="" class="submenu-link">Gestionadas</a>
                                </li>
                            </ul>
                        </li>
                        <li
                            class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-send-check-fill"></i>
                                <span>Memorandos</span>
                            </a>
                            <ul class="submenu ">
                                <li class="submenu-item">
                                    <a href="{{ route('admin.memorandos.emitidos') }}" wire:navigate class="submenu-link">Emitidos</a>
                                </li>
                                <li class="submenu-item">
                                    <a href="{{ route('admin.memorandos.recibidos') }}" wire:navigate class="submenu-link">Recibidos</a>
                                </li>
                            </ul>
                        </li>

                        <li class="sidebar-title">Estudiantes / Egresados</li>

                        <li
                            class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-person-fill"></i>
                                <span>Gestionar</span>
                            </a>

                            <ul class="submenu ">

                                <li class="submenu-item  ">
                                    <a href="{{ route('admin.estudiantes') }}" wire:navigate class="submenu-link">Estudiantes</a>
                                </li>
                                <li class="submenu-item  ">
                                    <a href="form-element-input.html" class="submenu-link">Egresados</a>
                                </li>
                            </ul>
                        </li>


                        <li class="sidebar-title">Profesores</li>

                        <li class="sidebar-item  has-sub">
                            <a href="#" class='sidebar-link'>
                                <i class="bi bi-person-circle"></i>
                                <span>Gestionar</span>
                            </a>

                            <ul class="submenu ">

                                <li class="submenu-item  ">
                                    <a href="{{ route('admin.profesores.crear') }}" wire:navigate class="submenu-link">Crear</a>

                                </li>

                                <li class="submenu-item  ">
                                    <a href="{{ route('admin.profesores') }}" wire:navigate class="submenu-link">Todos</a>

                                </li>
                            </ul>
                        </li>
                    </ul>
                    <div class="sidebar-footer mt-auto p-3 border-top border-gray-800">
                        <div class="dropdown">
                            <a href="#" class="d-flex align-items-center link-body-emphasis text-decoration-none dropdown-toggle"
                                id="dropdownUserMenu" data-bs-toggle="dropdown" aria-expanded="false" style="color: #eee;">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Usuario') }}&background=435ebe&color=fff"
                                    alt="mdo" width="32" height="32" class="rounded-circle me-2">
                                <div class="user-info">
                                    <h6 class="mb-0 text-sm font-bold">{{ auth()->user()->name ?? 'Usuario' }}</h6>
                                </div>
                            </a>
                            <ul class="dropdown-menu dropdown-menu-dark text-small shadow w-100" aria-labelledby="dropdownUserMenu">
                                <li><a class="dropdown-item" href="#"><i class="bi bi-person me-2"></i> Mi Perfil</a></li>
                                <li>
                                    <hr class="dropdown-divider">
                                </li>
                                <li>
                                    <form method="POST" action="{{ route('logout') }}" id="logout-form">
                                        @csrf
                                        <button type="submit" class="dropdown-item text-danger">
                                            <i class="bi bi-box-arrow-right me-2"></i> Cerrar Sesión
                                        </button>
                                    </form>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div id="main">
            <header class="mb-3">
                <a href="#" class="burger-btn d-block d-xl-none">
                    <i class="bi bi-justify fs-3"></i>
                </a>
            </header>

            <div class="page-heading">
                {{$slot}}
            </div>
            <footer>
                <div class="footer clearfix mb-0 text-muted">
                    <div class="float-start">
                        <p>2023 &copy; Mazer</p>
                    </div>
                    <div class="float-end">
                        <p>Crafted with <span class="text-danger"><i class="bi bi-heart-fill icon-mid"></i></span>
                            by <a href="https://saugi.me">Saugi</a></p>
                    </div>
                </div>
            </footer>
        </div>
    </div>
    @livewireScripts
    <script src="{{ asset('assets/extensions/perfect-scrollbar/perfect-scrollbar.min.js') }}"></script>
    <script src="{{ asset('assets/compiled/js/app.js') }}"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</body>

</html>